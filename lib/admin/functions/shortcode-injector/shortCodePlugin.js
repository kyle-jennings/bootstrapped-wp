...
function get_color()
{
	var style = jQuery('#galleryStyle').val();
	var size = jQuery('#thumbsSize').val();
	var wrapper = jQuery('#thumbsWrapper').val();
	var bgColor = jQuery('#hiddenField').val();	

	if(style=='thumbnails')
	{
		jQuery('#thumbsStyle').fadeIn();
		jQuery('#background-color').fadeOut();

		var display = '[gallery style="'+style+'" size="'+size+'" wrapper="'+wrapper+'"]';
	}else{
		jQuery('#thumbsStyle').fadeOut();
		jQuery('#background-color').fadeIn();

		var display = '[gallery style="'+style+'" color="'+bgColor+'"]';
...
// Inserts Code into page
		...
		jQuery("#insertCode").click(function(){
			...
			var shortcode = jQuery('#selectedShortCodeName').text();
