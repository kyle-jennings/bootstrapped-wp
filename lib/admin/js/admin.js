...
jQuery('.minicolors').minicolors({
	...
	change: function(hex, opacity) {
		...
		jQuery(this).parent().next('.rgba-color').val(text);
	}
});
...
// 		console.log(url); 
// 		jQuery(uploadBtn).prev('input').val(url); 
// 		jQuery(uploadBtn).next('.logo_preview img').attr('src',url); 
	...
	jQuery(".cycler_mgmt").sortable( {  
		...
		start: function(e, ui){
		    ...
		    jQuery(this).find('.tmce-active').each(function(){
		    	editorVal = jQuery(this).find('textarea').val();
		...
		stop: function(event, ui) {  
	        ...
	        jQuery(this).find('.tmce-active').each(function(){
	        	...
	        	jQuery(this).find('textarea').val() = editorVal;
