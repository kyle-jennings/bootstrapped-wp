...
function kjd_reload_styles(){
	var $preview = $('iframe'),
	preview_styles = object_name.root_url+'/lib/styles/preview.css';
	...
	var currentPage = $preview.contents().get(0).location.href ? $preview.contents().get(0).location.href : 'object_name.root_url' ;

	$preview.attr("src", currentPage).load(function(){
		$preview.fadeIn();
		$preview.contents().find('head').remove('#custom-css');
		$preview.contents().find('head').find('link[rel=stylesheet][href~="'+preview_styles+'"]').remove();
		$preview.contents().find('head').append('<link id="preview" href="'+preview_styles+'" rel="stylesheet" type="text/css" />');
...
$('.upload_image').click(function(e) {  
	...
	if('#add_image') {
		...
		var $frame = $uploadBtn.closest('.cycler_image').find('iframe').contents();
