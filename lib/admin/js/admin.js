...
$('.minicolors').each(function(){
	...
	$('.clearColor').click(function(){
		$(this).prev('.minicolors').find('input').val('');
		$(this).prev('.minicolors').find('.minicolors-swatch span').css('background','none');
	});
...
/* --------------------------------- Thick Box Media Uploader ---------------------------------------- */
	...
	// 	if($('.upload_option').is(':visible')){

	// 		window.send_to_editor = function(html) {  
	// 			var image_url = $('img',html).attr('src');  
	// 			$(uploadBtn).prev('input').val(image_url); 
	// 			$(uploadBtn).next('.logo_preview img').attr('src',image_url); 
	// 			tb_remove();  
	// 		}  
	// 	}
	//   return false;  
...
var uploadBtn, file_frame;

$('.upload_option').click(function(e) {  
	var uploadBtn = $(this);
    ...
    file_frame.on( 'select', function() {
      ...
      attachment = file_frame.state().get('selection').first().toJSON();
		...
		console.log(url); 
		$(uploadBtn).prev('input').val(url); 
		$(uploadBtn).next('.logo_preview img').attr('src',url); 
...
// function called by sortable to rename inputs
	function setInputName(element, id) {  
		...
		$(element).attr("id", newId);                 
	 	...
	 	// $(".text", element).attr("name", "kjd_cycler_images_settings[kjd_cycler_images]["+id+"][text]");
...
//makes cycler sortable
	$(".cycler_mgmt").sortable( {  
		...
		start: function(e, ui){
		    ...
		    $(this).find('.tmce-active').each(function(){
		    	editorVal = $(this).find('textarea').val();
		...
		stop: function(event, ui) {  
	        ...
	        $(this).find('.tmce-active').each(function(){
	        	...
	        	$(this).find('textarea').val() = editorVal;
